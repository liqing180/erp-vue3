...
onMounted(() => {
  ...
  addTags()
  setTimeout(() => {
    const scrollPaneElement = scrollPaneRef.value.$el.querySelectorAll(
      '.el-scrollbar__view'
    )[0]

    Sortable.create(scrollPaneElement, {
      handle: '.tags-view-item',
      animation: 150, // 动画持续时间
      onEnd(evt) {
        const movedItem = visitedViews.value.splice(evt.oldIndex, 1)[0]
        visitedViews.value.splice(evt.newIndex, 0, movedItem)
      }
    })
  }, 300)
})
