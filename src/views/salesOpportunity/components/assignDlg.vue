...
import { transferToSalesOpportunity } from '@/api/salesOpportunity/salesOpportunity'
...
export default {
  ...
  methods: {
    ...
    open(ids) {
      this.reset()
      const vm = this
      this.salesOpportunityIdList = ids
    ...
    queryCanSelectSalesGroupList() {
      queryCanSelectSalesGroupList({}).then(res => {
        ...
        this.salesGroupUserList = arr[0].salesGroupUserList
      })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          params.salesOpportunityIdList = this.salesOpportunityIdList
          ...
          this.$modal
            ...
            .then(() => {
              return transferToSalesOpportunity(params)
            })
            .then(response => {
