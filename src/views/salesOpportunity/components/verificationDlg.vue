  ...
  methods: {
    ...
    textFieldValidation(item) {
      ...
      if (validationType === 'equalTo') {
        ...
        // 验证包含：值不包含时不通过
        if (validationStrValue.indexOf(value) === -1) return true
    ...
    selectValidation(item) {
      ...
      const isContained = newValue.every(item =>
        newValidationStrValue.includes(item)
      )
    ...
    dateSelectionValidation(item) {
      ...
      const newValue = Math.abs((value - current) / 60 / 60 / 24 / 1000)
      if (newValue)
        if (validationType === 'equalTo') {
          // 验证等于：值不相等时不通过
