  ...
  <FormPageLayout>
    <template v-slot:btn>
      ...
      <el-button
        ...
        size="small"
        v-hasPermi="['customerManagement:customer:transfer']"
        ...
        <div class="p20">
          ...
          <div class="mt10 flexStart" style="font-weight: 700">
            ...
            <div
              ...
              v-if="page2CustomerIdList.length > 1"
            >
              <el-icon
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="4">
              <div class="fz-12 c-6 flow1">
                {{ $t('customer.numberOfCompletedSales') }}
              </div>
              <div
                class="mt10 flow1"
                v-if="form.customerSalesOpportunity.numberOfCompletedSales"
              >
                {{ form.customerSalesOpportunity.numberOfCompletedSales }}
              </div>
            </el-col>
            <el-col :span="4">
              <div class="fz-12 c-6">
                {{ $t('customer.amountOfCompletedSales') }}
              </div>
              <div class="mt10 flow1">
                {{
                  `${form.customerSalesOpportunity.currencySymbol || ''} ${$numberStr(form.customerSalesOpportunity.amountOfCompletedSales, 2)} `
                }}
              </div>
            </el-col>
            <el-col :span="4">
              <div class="fz-12 c-6 flow1">
                {{ $t('customer.lastPurchaseDate') }}
              </div>
              <div
                class="mt10 flow1"
                :title="
                  parseTime(
                    form.customerSalesOpportunity.lastPurchaseDate,
                    fmtForYmdhms
                  )
                "
              >
...
import SalesOpportunity from '@/views/customerManagement/customer/components/SalesOpportunity.vue'
...
export default {
  ...
  created() {
    ...
    const idsList = JSON.parse(sessionStorage.getItem('page2CustomerIdList'))
    this.page2CustomerIdList = idsList || []
  },
  activated() {
  ...
  computed: {
    ...
    editAuth() {
      return this.checkPermi(['customerManagement:customer:edit'])
    },
    comDisFrom() {
      if (this.isView) {
        return true
  ...
  methods: {
    leftAndRightChange(type) {
      const idsList = this.page2CustomerIdList
      ...
      this.$router
        .replace({
          path: '/redirect' + '/customerManagement/viewCustomer',
          query: {
            id,
            timeId: Date.now()
          }
    ...
    cancel() {
      ...
      const obj = { path: '/customerManagement/customer' }
    ...
    editForm() {
      this.$router.push({
        path: '/customerManagement/editCustomer',
        query: {
          id: this.rowId,
          timeId: Date.now()
        }
