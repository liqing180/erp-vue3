...
import alikeCustomerDlg from '@/views/customerManagement/prospectiveCustomer/alikeCustomerDlg.vue'
    ...
    submitForm: function () {
      this.$refs.formRef1.validate(valid1 => {
        ...
        this.$refs.formRef2.validate(valid2 => {
          ...
          if (!param.customerAddressList.some(item => item.isActive === '1')) {
            this.$modal.msgError(this.$t('customer.addressReq'))
          ...
          if (
            !param.customerContactPersonList.some(item => item.isActive === '1')
          ) {
            this.$modal.msgError(this.$t('customer.contactPersonReq'))
          ...
          if (!param.customerId) {
            queryLikeCustomerList(param).then(res => {
              ...
              if (data && data.length > 0) {
                ...
                this.$modal
                  .confirm(this.$t('customer.customerConfirm'))
                  .then(() => {
                    return saveCustomer(param)
                  })
                  .then(response => {
                    this.$modal.msgSuccess(
                      this.$t('customer.customerSuccess').replace(
                        '$1',
                        param.customerName
                      )
                    )
