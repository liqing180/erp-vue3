<template>
  <div>
    <div @click="addNews" style="height: 50px" v-show="!visible">
      <el-input
        :placeholder="$t('customer.writeFollowUp')"
        readonly
        value=""
        style="height: 50px"
      >
    ...
    <ul class="card mt10">
      ...
      >
        ...
        <div class="flexStart mt10" v-if="item.leadId">
          <el-icon
            class="mr10"
          ...
          <span class="mr5">{{ $t('customer.associatedLeads') }}</span>
          <span class="primary-pointer">{{ item.documentName }}</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    nextContactTimeDateChange() {
      ...
      this.$refs.form.validateField('nextFollowUpDay')
    },
    // 下次联系时间
