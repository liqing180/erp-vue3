...
export default {
  ...
  methods: {
    getList() {
      ...
      queryActivityList(params).then(response => {
    ...
    handleAdd() {
      this.$router.push({
        path: '/system/systemSetup/addActivity',
        query: {
          timeId: Date.now()
        }
      })
    ...
    handleUpdate(row, column) {
      ...
      this.$router.push({
        path: '/system/systemSetup/editActivity',
        query: {
          timeId: Date.now(),
          id: row.activityId
        }
    ...
    /** 删除按钮操作 */
    handleDelRow(row) {
      const idList = row.activityId ? [row.activityId] : this.ids
      this.$modal
        .confirm(
          this.$t('SALES.delActivityConfirm').replace('$1', row.activityName)
        )
        .then(function () {
          return deleteActivity({ selectIdList: idList })
        })
        .then(() => {
          this.getList()
