...
import { queryUserDefinedUserList } from '@/api/reportManagement/dashboard'
...
export default {
  ...
  methods: {
    ...
    // 显示弹框
    handleOpen(alreadySelectUserIdList) {
      // this.alreadySelectUserIdList = alreadySelectUserIdList || []
      this.selected = alreadySelectUserIdList
      this.queryParams.pageSize = this.$$initPageSize(this.saveKey)
      this.getList()
      this.visible = true
    },
    ...
    getList() {
      ...
      queryUserDefinedUserList({ condition }).then(res => {
