<template>
  ...
  <FormPageLayout>
      ...
      <el-collapse v-model="activeNames">
        ...
        <div class="form-card mt10">
          <el-collapse-item name="2">
            ...
            <AddressInfo ref="AddressInfo" :disabled="comDisFrom" />
          </el-collapse-item>
        </div>
        ...
        <div class="form-card mt10" v-if="!!form.leadId">
    ...
    <alikeLeadDlg ref="alikeLeadDlg" @onSuccess="cancel" />
  </FormPageLayout>
</template>
...
import alikeLeadDlg from './components/alikeLeadDlg'
      ...
      this.$refs.form.validate(valid => {
        ...
        if (valid) {
          this.$refs.form3.validate(valid3 => {
            ...
            if (!params.leadId) {
              queryLikeLeadList(params).then(res => {
                const { data } = res
                if (data && data.length > 0) {
                  this.$refs.alikeLeadDlg.handleAdd(data, params)
                } else {
                  this.$modal
                    ...
                    .then(response => {
                      this.$modal.msgSuccess(
                        this.$t('customer.leadSuccess').replace(
                          '$1',
                          params.leadName
                        )
                      )
