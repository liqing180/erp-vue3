...
const hideComponent1 = ref([])
const expression1 = ref('')
...
watch(expression1, () => resolveExp())
function shouldHide(key) {
  return !(hideComponent1.value && hideComponent1.value.includes(key))
}
function resolveExp() {
  // 反解析 表达式
  if (expression1.value) {
    const arr = expression1.value.split(/\s+/)
    if (arr.length >= 6) {
      //6 位以上是合法表达式
      let obj = {
        second: arr[0],
...
onMounted(() => {
  expression1.value = props.expression
  hideComponent1.value = props.hideComponent
})
</script>
